...
describe('setvalue action to populate defaults', () => {
    it('works for questions with odk-instance-first-load outside of the XForms body', () => {
        ...
        form1.init();
        expect(form1.model.xml.querySelector('a').textContent).to.equal(
            'initialized'
        );
    });
    ...
    describe('inside repeats', () => {
        describe('with only odk-new-repeat', () => {
            ...
            it('works when jr:repeat-count is used', () => {
                const form1 = new Form(
                    document
                        ...
                        .createContextualFragment(
                            `<div>${forms['setvalue.xml'].html_form.replace(
                                'class="or-repeat-info"',
                                'class="or-repeat-info" data-repeat-count="/data/count"'
                            )}</div>`
                        )
                        .querySelector('form'),
                    {
                ...
                expect([...ages].map((el) => el.textContent)).to.deep.equal([
                    '',
                    '100',
                    '5',
                    '5',
                ...
                expect([...ds].map((el) => el.textContent)).to.deep.equal([
                    '',
                    'bb',
                    'a',
                    'a',
        ...
        describe('with only odk-instance-first-load', () => {
            it('works for repeat questions inside default instances', () => {
                ...
                form2.view.html
                    .querySelector('[name="/data/person/age"]')
                    .setAttribute('data-event', 'odk-instance-first-load');
                form2.init();
            ...
            it('works for hidden repeat nodes inside default instances', () => {
                const form2 = loadForm('setvalue.xml');
                form2.view.html
                    .querySelector('[name="/data/person/d"]')
                    .setAttribute('data-event', 'odk-instance-first-load');
                form2.init();
        ...
        describe('with both odk-instance-first-load and odk-new-repeat events', () => {
            it('works for repeat questions inside default instances', () => {
                ...
                form3.view.html
                    ...
                    .setAttribute(
                        'data-event',
                        'odk-instance-first-load odk-new-repeat'
                    );
                form3.init();
            ...
            it('works for the first repeat if that repeat is non-relevant upon load', () => {
                ...
                form.init();
                const yn = form.view.html.querySelector(
                    '[name="/data/grp/yn"]'
                );
                form.input.setVal(yn, '1', events.Change());

                expect(
                    form.view.html.querySelector('[name="/data/grp/rep/pos"]')
                        .value
                ).to.equal('Standing');
            });
                ...
                expect(
                    [
                        ...form.view.html.querySelectorAll(
                            '[name="/data/grp/rep/pos"]'
                        ),
                    ].map((el) => el.value)
                ).to.deep.equal(['Standing', 'Sitting', 'Lying', '', '', '']);
            });
        });
    ...
    it('relying on non-form-control setvalue/odk-instance-first-load items to be evaluated before form-control setvalue items', () => {
        ...
        expect(form1.model.xml.querySelector('three').textContent).to.equal(
            '2#'
        );
    });
    ...
    it('relying on non-form-control setvalue/odk-new-repeat items inside repeats to be evaluated before form-control setvalue items', () => {
        ...
        form1.init();
        form1.view.html.querySelector('.add-repeat-btn').click();
        expect(form1.model.xml.querySelectorAll('one')[1].textContent).to.equal(
            '2'
        );
        expect(form1.model.xml.querySelectorAll('two')[1].textContent).to.equal(
            '2'
        );
        expect(
            form1.model.xml.querySelectorAll('three')[1].textContent
        ).to.equal('2#');
    });
});
...
describe('setvalue actions to populate a value if another value changes', () => {
    ...
    it('works outside a repeat in conjunction with a select_minimal', async () => {
        ...
        const myAgeChangedView = form.view.html.querySelector(
            '[name="/data/my_age_changed"]'
        );
        const myAgeChangedModel =
            form.model.xml.querySelector('my_age_changed');
    ...
    it('works inside a repeat in conjunction with a number input', async () => {
        ...
        form.init();
        const agesView = [
            ...form.view.html.querySelectorAll('[name="/data/person/age"]'),
        ];
        const ageChangedsView = [
            ...form.view.html.querySelectorAll(
                '[name="/data/person/age_changed"]'
            ),
        ];
        const ageChangedsModel = [
            ...form.model.xml.querySelectorAll('age_changed'),
        ];
    ...
    it('works if the setvalue trigger is a calculation', () => {
        ...
        expect(form.model.xml.querySelector('a_copy').textContent).to.equal(
            '1'
        );
        // check setvalue-changed value
        expect(form.model.xml.querySelector('triggered').textContent).to.equal(
            '11'
        );
    });
});
