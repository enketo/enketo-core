    ...
    describe('first load', () => {
        ...
        it('works for questions with odk-instance-first-load outside of the XForms body', (done) => {
            ...
            mock.lookup
                .then(({ geopoint }) => {
                    expect(
                        form1.model.xml.querySelector('hidden_first_load')
                            .textContent
                    ).to.equal(geopoint);
                })
                .then(done, done);
...
describe('setgeopoint actions to populate a value if another value changes', () => {
    ...
    it('works outside a repeat in conjunction with a select_minimal', (done) => {
        ...
        const changeTarget = form.view.html.querySelector(
            '[name="/data/changes"]'
        );
        const locationChangedView = form.view.html.querySelector(
            '[name="/data/location_changed"]:not([data-setgeopoint])'
        );
        const locationChangedModel = form.model.xml.querySelector(
            ...
            .then(() => {
                ...
                requestAnimationFrame(() => {
                    mock.lookup
                        .then(({ geopoint }) => {
                            expect(
                                form.input.getVal(locationChangedView)
                            ).to.equal(geopoint);
                            expect(locationChangedModel.textContent).to.equal(
                                geopoint
                            );
