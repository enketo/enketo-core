...
describe('ToC for pages mode', () => {
    describe('ToC generation', () => {
        ...
        it('builds a ToC that contains labels of question', () => {
            const form = loadForm('pages.xml');
            form.view.html.after(
                document
                    .createRange()
                    .createContextualFragment(`<ol class="pages-toc__list"/>`)
            );
            form.init();
            const toc = form.pages.$toc[0];

            expect(toc).not.to.equal(null);
            expect(toc.querySelectorAll('li[role="pageLink"]').length).to.equal(
                14
            );
            expect(
                [...toc.querySelectorAll('li[role="pageLink"]')].map(
                    (li) => li.textContent
                )
        ...
        it('updates a ToC when the language changes', () => {
            ...
            // Switch language
            const langSelector =
                form.view.html.parentNode.querySelector('#form-languages');
        ...
        it('uses an abbreviated hint or default [number] if page has no label', () => {
            ...
            expect(
                [...toc.querySelectorAll('li')].map((li) => li.textContent)
        ...
        it('builds a ToC with group as ToC item', () => {
            ...
            form.view.html.after(
                document
                    .createRange()
                    .createContextualFragment(`<ul class="pages-toc__list"/>`)
            );
            form.init();
            const toc = form.pages.$toc[0];

            expect(toc).not.to.equal(null);
            expect(toc.textContent).to.equal(
                'agroup onebcdegroup threefgthree-twohi'
            );
        });
    });
});
